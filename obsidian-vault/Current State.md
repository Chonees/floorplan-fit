...
- Auditor?a local verificada el **2026-04-29**: el ?nico commit versionado sigue siendo `chore: initialize floorplan-fit` (`ca19d38`); toda la base de Slice 1, `PLANS/`, `docs/`, `obsidian-vault/`, `src/` y `tests/` sigue como trabajo local no committeado
...
1. Reabrir la app desde `scripts/dev-desktop.bat` y confirmar visualmente que la Library reaparece desde SQLite al iniciar en una nueva sesion de UI
2. Reusar `PLANS/` como fuente inicial de fixtures DXF y catalogos para seguir TDD real sobre importacion, normalizacion y extraccion
3. Construir el primer vertical slice operativo sobre `SANTA-BARBARA`: importacion real -> unidades -> persistencia real -> library entry -> base para extraccion
