...
- **Windows**: cyan family
- **Doors**: base color aligned to the current seed original plan (`SEMINOLE2000.dxf` layer `DOORS` = `#455668`); selected state uses the unified selection green, not a per-family door highlight tint
- **Fixed elements**: red family by default, except **cabinets**, which use the same cyan family as windows even when DXF color metadata exists
- **Protected details**: preserve original DXF color when available; otherwise fall back to the fixed-element red family
...
## Consequences
- Semantic palette overrides DXF color for fixed elements so toilets, tubs and blocks stay visually consistent in Review, even though `ColorArgb` is still preserved in persistence.
- Opening geometry always uses semantic colors instead of neutral black.
- Protected details remain the family that can still render with original DXF color.
